...
$(document).on('click', '.table tbody tr td .btn-success', function () {
  ...
  else if(table_id.find('.txtjd_id').text().trim() === "DS001")
  ...
  else if(table_id.find('.txtjob_role').text().trim() === "Data Scientist")
...
function displayTable() {
  ...
  jobList.forEach(function (job) {
    ...
    text += "<td class='txtmax_exp'>" + (isNaN(job.maxExp) ? "" : job.maxExp) + "</td>";
...
if(isNaN(parseInt(min_exp)))
  ...
  jobList.push(new Job(jd_id, job_role, source, min_exp, max_exp, priority, category, keywords))
  displayTable()
